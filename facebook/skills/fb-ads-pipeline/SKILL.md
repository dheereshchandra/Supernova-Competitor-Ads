...
description: >-
  ...
  back-filled), then runs the fb-ads-video-analysis sub-skill (Creative Studio — scene
  ...
  estimate-and-approval step). Final master CSV has five Creative Studio URL columns —
  ...
  wants Steps 2+3 or only Creative Studio — invoke the relevant sub-skill directly
  instead.
---
...
- Operator only wants Creative Studio against existing master rows → use `fb-ads-video-analysis`
...
│  Identify candidate Creative Studio rows (missing both docx URLs) │
...
│                                                                   │
│  Phase D — Creative Studio (fb-ads-video-analysis)                │
│  ──────────────────────────────                                   │
...
3. **Sanity-check `.env`.** Preflight (step 0) already validates this, but double-check that both R2 credentials AND `GEMINI_API_KEY` are present. If `GEMINI_API_KEY` is missing, surface clearly:
   > *"Creative Studio requires a Gemini API key. Steps 2+3 will still run, but Creative Studio will be skipped. Continue with just Steps 2+3, or pause until the key is available?"*
...
If the sub-skill failed mid-way (any errors), surface the error and ask the operator whether to retry or stop. Don't push forward to Creative Studio if Step 3 didn't finish cleanly.
...
2. Tell the operator how many candidate rows exist and the default scope:
   > *"All Step 2+3 work is done. {competitor} master has {N} videos in R2, and {M} of them haven't been analysed yet. The default Creative Studio scope is the top 20 by row_rank (= most prominent active ads). I'll show you the cost estimate next — you can approve or adjust scope before anything fires."*
...
- **Default Creative Studio scope is `--top-by-rank 20`.** Don't ask the operator how many to run — just compute the estimate at that scope and show them the gate. They adjust there.
...
- **Never invoke `fb-ads-video-analysis` if Phase B failed.** Creative Studio has no value without Step 3's R2 URLs in the master.
